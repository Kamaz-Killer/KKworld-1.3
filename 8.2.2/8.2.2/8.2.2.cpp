...
struct Enemy {
    ...
    int x;
    int y;
    int stateTimer;   // обездвижен после удара
    int moveCooldown; // пауза между ходами
    bool active;
...
void StartNewGame(int difficulty, GameMode mode);
void UpdateFPS();
void CleanupGame();
...
std::vector<std::pair<int, int>> FindPathBFS(int startX, int startY, int endX, int endY);
...
void ConnectRooms()
{
    // Горизонтальные соединения - проход в 3 клетки
    for (int roomY = 0; roomY < ROOMS_Y; roomY++)
    {
    ...
    // Вертикальные соединения - проход в 3 клетки
...
void PlaceGoldAndItems()
{
    ...
    if (gameMode == INFINITE_MODE) healthCount = 4;
    ...
    int keyCount = (gameMode == INFINITE_MODE) ? 2 : (3 + rand() % 3);
...
bool IsWalkable(int x, int y)
{
    ...
    int cell = map[y * TOTAL_WIDTH + x];
    return cell == EMPTY || cell == PLAYER || cell == GOLD || cell == HEALTH ||
        cell == KEY || cell == DOOR || cell == FREE_DOOR || cell == EXIT_DOOR;
}

std::vector<std::pair<int, int>> FindPathBFS(int startX, int startY, int endX, int endY)
{
    std::vector<std::pair<int, int>> path;
    std::vector<std::pair<int, int>> parent(TOTAL_HEIGHT * TOTAL_WIDTH, { -1, -1 });
    std::queue<std::pair<int, int>> q;

    q.push({ startX, startY });
    parent[startY * TOTAL_WIDTH + startX] = { startX, startY };
    ...
    while (!q.empty())
    {
        ...
        if (x == endX && y == endY)
        {
            ...
            while (!(cx == startX && cy == startY))
            {
                ...
                auto [px, py] = parent[cy * TOTAL_WIDTH + cx];
                cx = px;
                cy = py;
            }
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (IsWalkable(nx, ny) && parent[ny * TOTAL_WIDTH + nx].first == -1)
            {
                parent[ny * TOTAL_WIDTH + nx] = { x, y };
                q.push({ nx, ny });
            }
        }
    }
...
{
    POINT playerPos = FindPlayer();
    if (playerPos.x == -1) return;

    int attempts = 0;
    while (attempts < 10)
    {
        int offsetX = (rand() % 3 - 1) * ROOM_WIDTH;
        int offsetY = (rand() % 3 - 1) * ROOM_HEIGHT;

        int spawnRoomX = currentRoomX + offsetX / ROOM_WIDTH;
        int spawnRoomY = currentRoomY + offsetY / ROOM_HEIGHT;

        if (spawnRoomX >= 0 && spawnRoomX < ROOMS_X && spawnRoomY >= 0 && spawnRoomY < ROOMS_Y)
        {
            int spawnX = spawnRoomX * ROOM_WIDTH + 5 + rand() % (ROOM_WIDTH - 10);
            int spawnY = spawnRoomY * ROOM_HEIGHT + 5 + rand() % (ROOM_HEIGHT - 10);
            ...
            if (IsWalkable(spawnX, spawnY))
            {
                ...
                enemy.stateTimer = 0;
                enemy.active = true;
                map[spawnY * TOTAL_WIDTH + spawnX] = ENEMY;
                return;
            }
        }
        attempts++;
...
void UpdateEnemy()
{
    ...
    {
        enemy.moveCooldown--;
        return; // пока кулдаун > 0, враг стоит
    }

    // здесь враг готов сделать ОДИН шаг
    POINT playerPos = FindPlayer();
    if (playerPos.x == -1) return;

    std::vector<std::pair<int, int>> path = FindPathBFS(enemy.x, enemy.y, playerPos.x, playerPos.y);
    ...
    // враг может открывать двери
    int cell = map[nextY * TOTAL_WIDTH + nextX];
    if (cell == DOOR || cell == FREE_DOOR)
        map[nextY * TOTAL_WIDTH + nextX] = EMPTY;

    // перемещение
    map[enemy.y * TOTAL_WIDTH + enemy.x] = EMPTY;
    enemy.x = nextX;
    enemy.y = nextY;
    ...
    if (enemy.x == playerPos.x && enemy.y == playerPos.y)
    {
        ...
        {
            DamagePlayer(20);
            enemy.stateTimer = 20 * 60 / 16; // ≈20 сек при таймере 16 мс
        }

        // ВАЖНО: враг ОСТАЁТСЯ на этой клетке, не стираем его с карты
        map[enemy.y * TOTAL_WIDTH + enemy.x] = ENEMY;

        // после удара враг не двигается, но не исчезает
        enemy.moveCooldown = 0;
        return;
    }

    // обычное состояние — враг просто занял новую клетку
    map[enemy.y * TOTAL_WIDTH + enemy.x] = ENEMY;

    // выставляем кулдаун до следующего шага
    enemy.moveCooldown = ENEMY_STEP_DELAY;
}

void ExpandMapIfNeeded()
{
    if (gameMode != INFINITE_MODE) return;

    // Если игрок достаточно близко к краю карты, расширяем лабиринт
    if (currentRoomX >= ROOMS_X - 2 || currentRoomY >= ROOMS_Y - 2)
    {
        // Создаем новые комнаты
        ROOMS_X++;
        ROOMS_Y++;

        int oldTotalWidth = TOTAL_WIDTH;
        int oldTotalHeight = TOTAL_HEIGHT;

        TOTAL_WIDTH = ROOM_WIDTH * ROOMS_X;
        TOTAL_HEIGHT = ROOM_HEIGHT * ROOMS_Y;

        int* newMap = new int[TOTAL_HEIGHT * TOTAL_WIDTH];
        bool* newExplored = new bool[ROOMS_Y * ROOMS_X];

        // Копируем старую карту
        for (int y = 0; y < oldTotalHeight; y++)
        {
            for (int x = 0; x < oldTotalWidth; x++)
            {
                newMap[y * TOTAL_WIDTH + x] = map[y * oldTotalWidth + x];
            }
        }

        // Заполняем новую область
        for (int y = oldTotalHeight; y < TOTAL_HEIGHT; y++)
        {
            for (int x = 0; x < TOTAL_WIDTH; x++)
            {
                newMap[y * TOTAL_WIDTH + x] = EMPTY;
            }
        }

        for (int y = 0; y < oldTotalHeight; y++)
        {
            for (int x = oldTotalWidth; x < TOTAL_WIDTH; x++)
            {
                newMap[y * TOTAL_WIDTH + x] = EMPTY;
            }
        }
        ...
        // Копируем explored
        for (int y = 0; y < ROOMS_Y - 1; y++)
        {
            for (int x = 0; x < ROOMS_X - 1; x++)
            {
                if (y < ROOMS_Y - 1 && x < ROOMS_X - 1)
                    newExplored[y * ROOMS_X + x] = explored[y * (ROOMS_X - 1) + x];
                else
                    newExplored[y * ROOMS_X + x] = false;
            }
        }
        ...
        for (int roomY = ROOMS_Y - 1; roomY < ROOMS_Y; roomY++)
        {
            for (int roomX = ROOMS_X - 1; roomX < ROOMS_X; roomX++)
            {
                GenerateRoom(roomX, roomY);
            }
        }

        // Соединяем новые комнаты
        EnsureAllRoomsConnected();
        PlaceDoors();
        PlaceGoldAndItems();

        // Переспавниваем врага
        enemySpawnCounter = 0;
    }
}
...
void DrawMap(HDC hdc)
{
    ...
    brushes[ENEMY] = CreateSolidBrush(RGB(255, 0, 0));
    ...
    if (playerPos.x != -1 && playerPos.y != -1)
    {
        float targetCameraX = playerPos.x * CELL_SIZE - bufferWidth / 2 + CELL_SIZE / 2 + UI_WIDTH / 2;
        float targetCameraY = playerPos.y * CELL_SIZE - bufferHeight / 2 + CELL_SIZE / 2;

        // Камера привязана к комнате
        int roomCenterX = (currentRoomX * ROOM_WIDTH + ROOM_WIDTH / 2) * CELL_SIZE;
        int roomCenterY = (currentRoomY * ROOM_HEIGHT + ROOM_HEIGHT / 2) * CELL_SIZE;

        cameraX = roomCenterX - (bufferWidth - UI_WIDTH) / 2.0f;
        cameraY = roomCenterY - bufferHeight / 2.0f;

        // Ограничиваем камеру
        int maxCameraX = TOTAL_WIDTH * CELL_SIZE - (bufferWidth - UI_WIDTH - MARGIN);
        int maxCameraY = TOTAL_HEIGHT * CELL_SIZE - bufferHeight + MARGIN;
    ...
    for (int y = 0; y < TOTAL_HEIGHT; y++)
    {
        for (int x = 0; x < TOTAL_WIDTH; x++)
        {
            ...
            int x1 = MARGIN + x * CELL_SIZE - (int)cameraX;
            int y1 = MARGIN + y * CELL_SIZE - (int)cameraY;
            int x2 = x1 + CELL_SIZE;
            int y2 = y1 + CELL_SIZE;
...
void DrawUI(HDC hdc)
{
    ...
    std::wstring modeStr = (gameMode == INFINITE_MODE) ? L"РЕЖИМ: БЕСКОНЕЧНЫЙ" : L"СТАТИСТИКА";
    TextOut(hdc, uiX, uiY, modeStr.c_str(), (int)modeStr.length());
    ...
    if (gameMode == INFINITE_MODE && enemy.active)
    {
        uiY += 25;
        SetTextColor(hdc, RGB(255, 0, 0));
        std::wstring enemyStatusStr = (enemy.stateTimer > 0) ? L"Враг: оглушен!" : L"Враг: активен!";
        TextOut(hdc, uiX, uiY, enemyStatusStr.c_str(), (int)enemyStatusStr.length());
    }
    ...
    uiY += 18;
    TextOut(hdc, uiX, uiY, L"O - Враг", 8);
    uiY += 18;
    ...
    uiY += 18;
    TextOut(hdc, uiX, uiY, L"- - Дверь (своб.)", 17);
    uiY += 18;
...
void DrawMenu(HDC hdc)
{
    ...
    wchar_t pressKey[] = L"Нажмите ENTER для выбора режима";
...
void DrawDifficultySelect(HDC hdc)
{
    ...
    wchar_t title[] = L"Выберите режим/сложность";
    SIZE titleSize;
    GetTextExtentPoint32(hdc, title, (int)wcslen(title), &titleSize);
    TextOut(hdc, (960 - titleSize.cx) / 2, 30, title, (int)wcslen(title));

    SelectObject(hdc, hFont);

    const wchar_t* diffNames[] = {
        L"1. ЛЕГКО (3x3)",
        L"2. НОРМАЛЬНО (4x4)",
        L"3. СЛОЖНО (5x5)",
        L"4. БЕСКОНЕЧНЫЙ РЕЖИМ"
    };
    ...
    const wchar_t* diffDescs[] = {
        ...
        L"Враг, динамическое расширение карты"
    };
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        TextOut(hdc, 200, y, diffNames[i], (int)wcslen(diffNames[i]));

        SetTextColor(hdc, RGB(150, 150, 150));
        TextOut(hdc, 240, y + 40, diffDescs[i], (int)wcslen(diffDescs[i]));
    }
...
void MovePlayer(int dx, int dy)
{
    ...
    case FREE_DOOR:
        map[playerPos.y * TOTAL_WIDTH + playerPos.x] = EMPTY;
        map[newY * TOTAL_WIDTH + newX] = EMPTY; // Удаляем дверь
    ...
    case DOOR:
        if (keys > 0 || godMode)
        {
            if (!godMode) keys--;
            map[playerPos.y * TOTAL_WIDTH + playerPos.x] = EMPTY;
            map[newY * TOTAL_WIDTH + newX] = EMPTY; // Удаляем дверь полностью
            roomX = newX / ROOM_WIDTH;
            roomY = newY / ROOM_HEIGHT;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
    {
        ...
        case DIFFICULTY_SELECT:
            ...
            else if (wParam == VK_DOWN && selectedDifficulty < 4)
            {
                selectedDifficulty++;
                InvalidateRect(hWnd, NULL, FALSE);
            }
            ...
            {
                GameMode mode = (selectedDifficulty == 4) ? INFINITE_MODE : NORMAL_MODE;
                StartNewGame((selectedDifficulty == 4) ? 1 : selectedDifficulty, mode);
                InvalidateRect(hWnd, NULL, FALSE);
            }
            break;
    ...
    case WM_TIMER:
        UpdateFPS();
        if (gameMode == INFINITE_MODE && gameState == PLAYING)
        {
            enemySpawnCounter++;
            if (enemySpawnCounter > ENEMY_SPAWN_DELAY && !enemy.active)
            {
                InitializeEnemy();
            }
            if (enemy.active)
